...
public class WWShipTest {
	...
	public void test() {
		...
		HtmlTableRow row2 = table.getRow(9);
		HtmlTableCell cell3 = row2.getCell(5);
		//System.out.println(cell3.asNormalizedText());
		//assert ship this order present
		Assert.assertTrue(cell3.asNormalizedText().equals("Ship This Order"));
		
		List<HtmlAnchor> anchors = page3.getAnchors();
		
		int ShipOptions =0;
		for(int i=0; i<anchors.size(); i++) {
	if(anchors.get(i).asNormalizedText().equals("Ship This Order")) {
		ShipOptions++;
	}
}
		
//Asserts that Ship this order is available on the page for warehouse workers:	
		Assert.assertTrue(ShipOptions>0);
