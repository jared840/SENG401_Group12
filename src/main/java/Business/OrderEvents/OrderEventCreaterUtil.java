...
public class OrderEventCreaterUtil {
	...
	public OrderEvent CreateOrderEventFromGUI(DBController db,Map<String, String[]> paramMap) throws Exception {
		  ...
		  OrderEvent event = new OrderEvent(
				  					...
				  					eventType, 
				  					new Date(),
				  					paramMap.get("userId")[0],
				  					paramMap.get("page")[0]);
		  
		if(event.getClassType().equals(OrderEventTypes.ADD_TO_CART.toString())) {
			return new AddToCartEvent(event, db.getProductById(Integer.parseInt(paramMap.get("productId")[0])), Integer.parseInt(paramMap.get("quantity")[0]));

		} else if(event.getClassType().equals(OrderEventTypes.REMVOVE_FROM_CART.toString())) {
			return new RemoveFromCartEvent(event, db.getProductById(Integer.parseInt(paramMap.get("productId")[0])), Integer.parseInt(paramMap.get("quantity")[0]));
		} else {
